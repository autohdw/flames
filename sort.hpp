...
static void mergeSort(V& vec) {
    ...
    MERGE_SORT_STAGE: for (int width = 1; width < size; width *= 2) {
        int f1 = 0;
        int f2 = width;
        int i2 = width;
        int i3 = 2 * width;
        if (i2 >= size) i2 = size;
        if (i3 >= size) i3 = size;
        MERGE_ARRAYS: for (int i = 0; i < size; ++i) {
        ...
        MERGE_SORT_COPY: for (int i = 0; i < size; ++i) {
...
static inline void argmax_4_2(T in1, T in2, T in3, T in4, I i_in1, I i_in2, I i_in3, I i_in4, T& out1, T& out2, I& i_out1, I& i_out2, bool sorted = false) {
